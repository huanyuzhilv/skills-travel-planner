...
copy_skill_to() {
  ...
  rm -rf "${target}/assets/templates"
  cp -R "${SCRIPT_DIR}/assets/templates" "${target}/assets/templates"
...
run_doctor() {
  ...
  for dir in \
    ...
    if [[ -f "${dir}/skill.md" ]]; then
      found_any=true
      printf "    ${GREEN}✓${NC} %s\n" "${dir}"
      for f in preview.html generate.py; do
      ...
      if [[ -f "${dir}/assets/templates/default/template.html" ]]; then
        printf "      ${GREEN}✓${NC} assets/templates/ (HTML templates)\n"
      else
        printf "      ${RED}✗${NC} assets/templates/default/template.html ${DIM}missing${NC}\n"
      fi
    fi
  done
